...
def main():
	...
	global source, pickle_result, json_result, jsonlines_result
	...
	jsonlines_time = timeit.Timer('test_jsonlines()', 'from __main__ import test_jsonlines, source, jsonlines_result')
	...
	jsonlines_load_time = timeit.Timer('test_jsonlines_load()', 'from __main__ import test_jsonlines_load, source, jsonlines_result')
	...
	for i in (10, 20, 50, 100):
		...
		print ("dump\t%s\tJSONl\t%.3f\t%s" % (i, jsonlines_time.timeit(i), len(''.join(str(jsonlines_result)))))
		print ("load\t%s\tJSONl\t%.3f\t%s" % (i, jsonlines_load_time.timeit(i), '-'))
		print ("dump\t%s\tPickle\t%.3f\t%s" % (i, pickle_time.timeit(i), len(''.join(str(pickle_result)))))
		#print ("dump\t%s\tPickle\t%.3f\t" % (i, pickle_time.timeit(i)))
...
def test_jsonlines():
	"""
		Runs the dumps JSONlines test.
	"""
	for entry in source:
		jsonlines_result.append(jsonlines.Reader(entry))
...
def test_jsonlines_load():
	"""
		Runs the loads JSON test.
	"""
	for entry in jsonlines_result:
		jsonlines.Reader(entry)
